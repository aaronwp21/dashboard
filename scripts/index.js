...
async function getWeather() {
  ...
  try {
    ...
    const response = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=d164cbfef0cd4d0b9e7200203230302&q=${latitude},${longitude}&days=1&aqi=no&alerts=no`);
...
function renderJoke(data){
  ...
  const joke = data[randomNum];
  jokeWrapper.innerHTML = `<p class="section-titles">Daily Joke</p>
  <p class="section-info">${joke.setup}</p>
  <button id="joke-button" class="section-button">Reveal</button>`
  ...
  jokeButton.addEventListener("click", (e) => {
    ...
    jokeWrapper.innerHTML = `<p class="section-titles">Daily Joke</p>
  <p class="section-info">${joke.setup}</p>
  <p class="joke-answer">${joke.punchline}</p>`
  });
}
...
function renderFootball(data){
  ...
  data.response.forEach((fixture) => {
    ...
    homeDiv.innerHTML = 
    `<img class="team-image" src="${fixture.teams.home.logo}" alt="${fixture.teams.home.name} logo">
    ...
    awayDiv.innerHTML = 
    `<img class="team-image" src="${fixture.teams.away.logo}" alt="${fixture.teams.away.name} logo">
...
async function getFootball(){
  ...
  try {
    const response = await fetch(`https://v3.football.api-sports.io/fixtures?league=39&season=2022&next=7`, {
      method: "GET",
      headers: {
